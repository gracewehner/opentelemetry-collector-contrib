...
func NewManager(set receiver.Settings, cfg *Config, registry *prometheus.Registry, registerer prometheus.Registerer) *Manager {
	return &Manager{
...
func (m *Manager) Start(ctx context.Context, host component.Host, promCfg *promconfig.Config, scrapeManager *scrape.Manager) error {
	m.settings.Logger.Info("Starting Prometheus API server")
	m.scrapeManager = scrapeManager
	m.ApplyConfig(promCfg)
