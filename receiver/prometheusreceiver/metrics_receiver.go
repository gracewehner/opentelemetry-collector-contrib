...
func newPrometheusReceiver(set receiver.Settings, cfg *Config, next consumer.Metrics) (*pReceiver, error) {
	...
	if cfg.APIServer != nil && cfg.APIServer.Enabled {
		apiServerManager = apiserver.NewManager(set, cfg.APIServer, registry, registerer)
	}
	pr := &pReceiver{
		cfg:          cfg,
		consumer:     next,
...
func (r *pReceiver) Start(ctx context.Context, host component.Host) error {
	...
	err = r.targetAllocatorManager.Start(ctx, host, r.scrapeManager, r.discoveryManager, r.apiServerManager)
	if err != nil {
		return err
	}
...
func (r *pReceiver) initPrometheusComponents(ctx context.Context, logger *slog.Logger, host component.Host) error {
	...
	if r.apiServerManager != nil {
		...
		err = r.apiServerManager.Start(ctx, host, &promCfg, r.scrapeManager)
		if err != nil {
